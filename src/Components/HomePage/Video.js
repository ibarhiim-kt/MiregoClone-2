  ...
  return (
    <div className='px-10 max-md:px-4 max-[424px]:px-2 gsapVideoTrigger'>
      ...
      <div className='flex items-center justify-center w-full'>
        <button
          className='flex justify-center items-center overflow-hidden w-[20%] rounded-[32px] videoAnimation max-lg:rounded-3xl gsapVideoAnima cursor-none'
      ...
      <div className='flex gap-2 pt-2 font-AlmiregoDisplayLight max-lg:flex-wrap max-md:flex-col cardsTrigger overflow-hidden'>
        {cards.map((map, index) => (
          <div
            key={index}
            className={`bg-[#f7edde] p-8 w-[25%] rounded-[32px] max-lg:rounded-3xl max-lg:w-[calc(50%-4px)] max-md:w-[100%] max-md:p-7 max-[424px]:p-7 animaCard${index + 1}`}>
            <div>
              <p className='mb-4 leading-[105%] mt-[38px] max-md:mt-[21px] max-[424px]:mt-[15px] max-[320px]:mt-2'>
                <span className='text-[88px] tracking-[-1.75px] max-xl:text-7xl max-md:text-[56px] max-[424px]:text-[40px] max-[320px]:text-[32px]'>{map.number}</span>
              ...
              <p className="text-[21px] leading-[125%] mt-11 tracking-[0.4px] font-AlmiregoRegular max-md:text-lg max-md:mt-[26.6px] max-[424px]:mt-[22.2px] max-[390px]:text-[17px] max-[320px]:mt-[21.2px]">
                {map.text}
              </p>
            </div>
          </div>

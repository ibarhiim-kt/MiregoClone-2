...
export default function BlogTable() {
  ...
  return (
    <div className='pt-52 font-AlmiregoRegular my-2 max-xl:pt-40 max-lg:pt-32 max-md:pt-24 min-1440:pt-64'>
      <div className='text-#050307 flex items-stretch justify-between gap-2 mb-2 max-lg:hidden card1'>
        <div className="py-11 px-10 text-18 flex flex-1 gap-154 bg-#F7EDDE rounded-3xl max-xl:gap-4">
            <p className='flex flex-col justify-between' style={{flex:1}}>DATE</p>
            <div style={{flex:2}}>NEWS</div>
        </div>
        <div className='flex items-center justify-center w-60 bg-#F7EDDE rounded-3xl  cursor-pointer'>
            <p className='mr-1 font-AlmiregoDisplayBold'>SEE ALL</p>
            <img src={blackArrow} alt="arrow" />
        </div>
      </div>
    {blogTable.map((map,index)=>(
      <div className={`text-#050307 flex items-stretch justify-between gap-2 mb-2
      cursor-point max-md:flex-col ${index === 0?"card2":index===1?"card3":"card4"}`}>
        <div className="py-11 px-10 flex flex-1 gap-154 bg-#F7EDDE rounded-3xl min-h-60 max-xl:gap-4 max-lg:flex-col max-lg:p-6">
            <div className='flex flex-col justify-between flex-1 max-md:flex-none'>
            <p className='flex text-18 flex-col justify-between max-lg:text-[16px] max-lg:mb-4 max-md:mb-3'>{map.date}</p>
            <ul className='flex'>
            <li className='rounded-[9.5px] py-[2.55px] px-[12.387px] border border-#050307 text-[19px]'>{map.blogText}</li>
            </ul>
            </div>
            <div className="text-32 max-lg:text-24" style={{flex:2}}>
            <h2  className='leading-[1.175]'>{map.para} </h2>
            </div>
        </div>
        <div className='overflow-hidden flex items-center justify-center max-w-60 bg-#F7EDDE rounded-3xl max-md:max-w-none'>            

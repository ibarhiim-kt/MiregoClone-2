...
const StickyCards = () => {
  ...
  return (   
      ...
      <div className='py-16 flex justify-between gap-2 max-xl:pt-14 max-lg:pt-12 max-md:py-8 max-md:flex-col max-xxs:pt-6'>
        ...
        <p className='home-para pb-[51px] distinctiveParaAnima max-xl:pb-[43px] max-lg:pb-[35px]'>Through the years, we've built digital products that millions of people use every day on phones, web browsers, tablets, connected watches, TVs, kiosks, and voice assistants.</p>
      </div>    
      </div>
      <div className="relative pt-16 -translate-y-11 -mb-36 max-xl:pt-14 max-lg:pt-12 max-md:pt-8 max-xxs:pt-6 flex flex-col ">
          ...
          <div className={`cursor-pointer group aspect-[7/3] sticky overflow-hidden p-8 pb-0 rounded-4xl border-2 border-white max-xl:rounded-3xl max-md:rounded-4xl max-md:aspect-[1/1] max-md:pt-7 max-md:px-7 max-s:p-6 max-s:pb-0 ${index===3?"bg-black":''}`} style={{ top:`${40 + (index + 1) * 100}px`,marginBottom:`${(4 - index)*100}px`, marginTop:`${(4-map.mt)*-100}px` }}>
            ...
            <div className="flex flex-col justify-between w-full h-full">           
              <div className={`flex items-center justify-between ${index<3?"absolute":"static"} left-0 top-0 w-full ${index<3?"p-8 max-md:p-7 max-s:p-6":"p-0"} leading-relaxed-xl`}>

            <div className={`font-AlmiregoDisplayRegular text-base text-white-primary flex items-center gap-2 
              ${index<3?"max-lg:text-sm-base max-s:text-[20px] max-xxs:text-small":''}`}>
              {map.name}              

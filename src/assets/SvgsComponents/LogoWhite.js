...
const AnimatedLogo = () => {
    ...
    const maskAnimRef = useRef(null);
    const maskAnimReverseRef = useRef(null);    
    const miniLogoMorphReverseRef = useRef(null);    
    ...
    useEffect(() => {
        if (isScrolling) {
            setTimeout(() => {
                        if (maskAnimRef.current) {
                            maskAnimRef.current.beginElement();
                        }                    
                        });                    
        } else {
            setTimeout(() => {
                     
                       if (maskAnimReverseRef.current) {
                         maskAnimReverseRef.current.beginElement();
                     }     
                     
                     if(miniLogoMorphReverseRef.current){                 
                        miniLogoMorphReverseRef.current.beginElement();
                      }        
                   });
        }
    }, [isScrolling]);
  return (  
    ...
    >
      ...
      >
        <animate
          id="minilogoanimscale"                
          fill="freeze"
          begin="maskanim.begin"
        ...
        <animate
          id="miniLogoAnimMorph"         
          fill="freeze"
          begin="minilogoanimscale.end"
          attributeName="d"
          dur="400ms"
          to="M 112.856 7.679 C 108.691 3.509 102.411 0.52 95.921 0.035 L 92.355 0 C 86.175 0.437 79.816 3.186 75.686 7.041 C 71.552 10.895 68.226 18.369 67.361 24.5 L 63.519 79.676 C 63.523 79.742 63.694 80.651 62.544 81.717 C 61.399 82.788 59.175 82.788 58.025 81.722 C 56.88 80.655 57.081 79.842 57.051 79.685 L 52.235 12.285 L 51.5 1.989 L 0 1.989 L 0 104.71 L 22.18 104.71 L 22.18 24.168 C 23.624 24.168 25.069 24.168 26.513 24.168 C 27.957 24.168 29.402 24.168 30.846 24.168 L 34.928 81.263 C 35.78 87.093 38.979 94.295 42.913 97.953 C 46.842 101.615 54.075 104.281 59.948 104.718 L 60.495 104.718 C 66.377 104.281 73.741 101.607 77.675 97.94 C 81.608 94.269 84.798 87.058 85.646 81.219 L 89.488 26.043 C 89.706 24.089 91.454 22.092 93.683 22.127 C 93.855 22.127 94.027 22.127 94.199 22.127 C 96.153 22.044 98.342 23.77 98.408 26.336 L 98.408 104.71 L 120.587 104.71 L 120.587 104.71 L 120.587 26.336 C 120.102 19.846 117.025 11.844 112.856 7.679"
        />
    <animate id="miniLogoAnimMorphReverse" ref={miniLogoMorphReverseRef} fill="freeze" attributeName="d" dur="400ms" to="M 118.583 1.759 C 113.912 1.759 109.241 1.759 104.57 1.759 L 90.558 1.759 C 85.887 1.759 81.216 1.759 76.545 1.759 C 74.283 14.817 72.021 27.875 69.759 40.933 L 60.973 79.108 C 60.9 79.51 60.699 79.839 60.416 80.068 C 60.133 80.296 59.767 80.424 59.365 80.424 C 58.561 80.424 57.903 79.839 57.756 79.108 L 49.97 40.434 L 42.184 1.759 L 0 1.759 L 0 98.847 L 21.348 98.847 L 21.348 22.083 C 21.348 21.206 22.079 20.402 23.03 20.402 C 23.834 20.402 24.492 20.913 24.638 21.718 L 32.717 60.283 C 35.409 73.138 38.102 85.992 40.795 98.847 C 44.905 98.847 49.016 98.847 53.126 98.847 L 65.457 98.847 C 69.567 98.847 73.678 98.847 77.788 98.847 C 79.583 90.277 81.378 81.707 83.174 73.138 L 88.559 47.428 C 90.355 38.858 92.15 30.288 93.945 21.718 C 94.091 20.986 94.749 20.402 95.554 20.402 C 96.431 20.402 97.235 21.133 97.235 22.083 L 97.235 98.847 L 107.909 98.847 L 118.583 98.847 L 118.583 1.759 C 118.583 1.759 118.583 1.759 118.583 1.759"></animate></path><animate id="maskanimReverse" begin="miniLogoAnimMorphReverse.end" attributeName="width" from="0%" to="100%" xlinkHref="#mask-fill" dur="300ms" fill="freeze"></animate>   
    </svg>
  );
};
